...
class _ShopHomePageState extends State<ShopHomePage> {
  ...
  Future<void> searchItems(String query, {String? filter}) async {
    ...
    final itemsUrl = Uri.parse(
            '${dotenv.env['API_BASE_URL']}/GP/v1/seller/items/searchItems')
  ...
  Future<void> fetchItems() async {
    final itemsUrl = Uri.parse(
        '${dotenv.env['API_BASE_URL']}/GP/v1/seller/items/getAllItems');
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('jwt_token');
  ...
  Future<void> fetchItemsByCategory(String category) async {
    ...
    final itemsUrl = Uri.parse(
            '${dotenv.env['API_BASE_URL']}/GP/v1/seller/items/getItemsByCategory')
  ...
  Future<List<Map<String, dynamic>>?> fetchNotifications(int userId) async {
    try {
      ...
      final String apiUrl =
          '${dotenv.env['API_BASE_URL']}/GP/v1/notification/$userId';
      final response = await http.get(
        Uri.parse(apiUrl),
        headers: {
          'Content-Type': 'application/json',
  ...
  Future fetchNotificationCount(int userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print(
            'Notification count fetched successfully: ${data['notificationCount']}');
        setState(() {
  ...
  Future<void> fetchStudentItems() async {
    final itemsUrl = Uri.parse(
        '${dotenv.env['API_BASE_URL']}/GP/v1/seller/items/getItemsForStudent');
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('jwt_token');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print(
            data); // Add this line to print the raw response to check its structure
        if (data != null &&
            data is Map<String, dynamic> &&
            data.containsKey('items')) {
          setState(() {
                  ...
                  onSelected: (value) {
                    if (value == "No Filter") {
                      ...
                      searchItems(_searchController.text,
                          filter: value); // Pass the selected filter
                    }
                  },
                  itemBuilder: (context) => [
          ...
          isLoading
              ...
              : items.isEmpty
                  ...
                  : GridView.builder(
                      shrinkWrap: true,
                      physics:
                          NeverScrollableScrollPhysics(), // Disable independent scrolling
                      gridDelegate:
                      ...
                      itemBuilder: (context, index) {
                        final item = items[
                            index]; // Make sure this is a Map<String, dynamic>
...
class _ItemCardState extends State<ItemCard> {
  ...
  Future<void> checkIfFavorite() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            '${dotenv.env['API_BASE_URL']}/GP/v1/shop/favoriteItems/checkFavoriteItem/${widget.item['Item_ID']}'),
        headers: {'Authorization': 'Bearer $token'},
      );
    ...
    try {
      if (isFavorite) {
        ...
        final response = await http.delete(
          Uri.parse(
              '${dotenv.env['API_BASE_URL']}/GP/v1/shop/favoriteItems/removeFavoriteItem'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
          },
        ...
        final response = await http.post(
          Uri.parse(
              '${dotenv.env['API_BASE_URL']}/GP/v1/shop/favoriteItems/addFavoriteItem'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
          },
  ...
  Future<void> addToCart(
      BuildContext context, int itemId, int quantity, int price) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('jwt_token');
    ...
    try {
      final response = await http.post(
        Uri.parse(
            '${dotenv.env['API_BASE_URL']}/GP/v1/shop/cart/createOrUpdateCart'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
                      ...
                      Row(
                        children: [
                          IconButton(
                            ...
                            onPressed:
                                toggleFavorite, // Call toggleFavorite on press
                          ),
                          IconButton(
                            icon: Icon(Icons.shopping_cart_outlined,
                                color: Color(0xFF3B4280), size: 20),
                            onPressed: () {
                              addToCart(
                                  context, widget.item['Item_ID'], 1, price);
                            },
                          ),
                        ],
                      ),
